...
const AlumniGraphModal: React.FC<Props> = ({
  visible,
  onClose,
  data,
  embedded = false,
  ...
  useEffect(() => {
    ...
    const width = embedded ? svgRef.current.clientWidth || 500 : 800;
    const height = embedded ? 500 : 600;
    ...
    const node = svg
      ...
      .attr('fill', (d) =>
        d.group === 'distinguished' ? '#d9363e' : '#1890ff',
      ) // 杰出校友红色，普通蓝色
  ...
  }, [visible, data, embedded]);

  const svgContent = (
    <svg
      ref={svgRef}
      width={embedded ? '100%' : 800}
      height={embedded ? 500 : 600}
